...
function initialize_belief(updater::InteractiveBeliefUpdater{S}, ipomdp::IPOMDP{S}, agent::Agent{S,A,W}, frame::ipomdpFrame{S,A,W}) where {S,A,W}
    # This map will contain all the data needed in order to avoid repetitive calculations during execution time
    ...
    # Return all the data needed to generate the belief
